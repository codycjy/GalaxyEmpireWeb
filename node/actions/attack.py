...
def attack_action(task: Task, result_queue: Queue):
    ...
    try:
        ...
        return
    result = TaskResult(task_id=task.task_id, status=task_status, task_type=task.task_type, back_ts=back_ts, uuid=uuid)
...
def explore_action(task: Task, result_queue: Queue):
    ...
    try:
        ...
        print(f"Error processing task: {e}")
        return
    result_queue.put(TaskResult(task_id=task.task_id, status=task_status, task_type=task.task_type, back_ts=back_ts, uuid=task.uuid))
